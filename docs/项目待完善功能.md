...
#### factsheetRequest 行为说明

- 作用：收到 `factsheetRequest` 即刻向 `uagv/{version}/{manufacturer}/{serialNumber}/factsheet` 广播一次 factsheet。
- 频率：仿真车仅在启动时自动发送一次 factsheet，不做定时发送；如需再次获取，请下发 `factsheetRequest`。
- 时间戳：支持 `timestamp` 与 `timeStamp` 两种键名。
